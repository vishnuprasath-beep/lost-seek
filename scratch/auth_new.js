  ...
  try {
    if (action === 'login') {
      ...
      if (!signInError && signInData.session) {
        ...
        actualRole = (supaUser.user_metadata && supaUser.user_metadata.role) || 'student';
        
        let dbUser = await db.getUser(cleanUser);
        if (!dbUser) dbUser = await db.getUser(email);
        
        authenticatedUser = {
          id: (supaUser.user_metadata && supaUser.user_metadata.legacy_id) || supaUser.id,
          username: cleanUser,
        ...
        const { data: createData, error: createError } = await supabase.auth.admin.createUser({
          ...
          password: cleanPass,
          email_confirm: true,
          user_metadata: { role: actualRole, name: authenticatedUser.name, legacy_id: authenticatedUser.id }
        });
    ...
    if (action === 'register') {
      ...
      if (safeAvatarUrl && safeAvatarUrl.startsWith('data:') && safeAvatarUrl.length > 200 * 1024) {
        return res.status(400).json({ success: false, message: 'Profile image exceeds limit.' });
      }
      ...
      const { data: authData, error: authError } = await supabase.auth.admin.createUser({
        ...
        password: rawPass,
        email_confirm: true,
        user_metadata: { role: safeRole, name: cleanName, legacy_id: legacyId }
      });
    ...
    if (action === 'profile') {
      ...
      if (req.method === 'POST' || req.method === 'PATCH') {
        ...
        const avatar = body.avatarUrl || body.avatar_url || body.profilePicture || body.photoUrl;
        if (avatar !== undefined) updates.avatarUrl = String(avatar).trim();
